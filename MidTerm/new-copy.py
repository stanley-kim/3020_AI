...
def GA_without_threads2(population_size=10, gene_count=3, generation_count=1000, motor_update_count=2400):
    print(f'population_size({population_size}), gene_count({gene_count}), generation_count({generation_count}), motor_update_count({motor_update_count})')
...
#            sim.run_creature(cr, 2400)            
        ...
        for i in range(len(pop.creatures)):
            ...
            dna = genome_with_fixed_body.Genome_with_fixed_body.point_mutate2(dna, rate=0.1, amount=0.25)
        ...
        for cr in pop.creatures:
            if cr.get_distance_travelled() == max_fit:
                ...
                new_creatures[0] = new_cr   
                filename = os.path.join(DIRNAME_CSVS, "elite_FB_"+ "P" + str(population_size) + "_" + "GC" + str(gene_count)  + "_" + "IT" + str(iteration).zfill(5)+"_" + str(np.round(max_fit, 3))  +".csv")
...
#    GA_without_threads2(population_size=10, gene_count=4, generation_count=400, motor_update_count=2400)
...
#    for ps in [10, 20, 30, 40, 50]:
    for ps in [10, 20,]:
        ...
        for gc in [3, 5]:
            for genc in [10, 20]:
                GA_without_threads(population_size=ps, gene_count=gc, generation_count=genc, motor_update_count=2400)
